...
# 이중 리스트 형태의 comment_tag에서 'c타입' 관련 키워드가 포함된 텍스트 추출하는 함수
def extract_relevant_tags(comment_tags):
    ctype_keywords = ['c타입', 'ctype', 'typec', '타입c', 'type-c']
    try:
        tags_list = ast.literal_eval(comment_tags)  # 문자열을 실제 리스트로 변환
        relevant_texts = []
        for tag_tuple_list in tags_list:
            if any(keyword in word for keyword in ctype_keywords for word, pos in tag_tuple_list):
...
# comment_tag에서 'c타입' 관련 키워드가 포함된 텍스트 추출
filtered_df2 = df.copy()
filtered_df2['extracted_text'] = filtered_df2['comment_tag'].apply(extract_relevant_tags)

# NaN 값 제거 (필터링된 텍스트가 없는 경우 제외)
filtered_df2 = filtered_df2.dropna(subset=['extracted_text'])

# comment_text에 대해 전처리 적용
filtered_df2['comment_text'] = filtered_df2['comment_text'].apply(compress_punctuation)

# 'published_at'을 datetime 형식으로 변환
filtered_df2['published_at'] = pd.to_datetime(filtered_df2['published_at'], errors='coerce')
...
filtered_df2['ctype_sentiment_label'], filtered_df2['ctype_sentiment_score'] = zip(
    *filtered_df2['extracted_text'].apply(analyze_sentiment_after_stopwords_removal)
)

# 결과 CSV 파일을 같은 폴더에 저장
output_file_path = os.path.join(current_directory, 'applepay_comments_with_sentiment.csv')
filtered_df2.to_csv(output_file_path, index=False, encoding='utf-8-sig')

print(f"c타입 관련 감정 분석이 완료되었으며, 결과가 {output_file_path}에 저장되었습니다.")
